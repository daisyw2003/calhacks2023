...
import { Box, Button, Center, Checkbox, Divider, HStack, Image, Input, Link, Menu, MenuButton, MenuDivider, MenuItem, MenuList, Modal, ModalBody, ModalContent, ModalHeader, ModalOverlay, Spacer, Text, VStack } from "@chakra-ui/react"
...
export default function Component() {
  ...
  const ref_get = useRef(null); refs['ref_get'] = ref_get;
  const ref_email = useRef(null); refs['ref_email'] = ref_email;
  const ref_date = useRef(null); refs['ref_date'] = ref_date;
  const ref_end = useRef(null); refs['ref_end'] = ref_end;
  const ref_start = useRef(null); refs['ref_start'] = ref_start;
  const ref_name = useRef(null); refs['ref_name'] = ref_name;
  const ref_task = useRef(null); refs['ref_task'] = ref_task;
  ...
  <Box as={`form`} onSubmit={(_e0) => addEvents([Event("state.form_state.handle_submit", {form_data:{"task": getRefValue(ref_task), "name": getRefValue(ref_name), "email": getRefValue(ref_email), "end": getRefValue(ref_end), "get": getRefValue(ref_get), "date": getRefValue(ref_date), "start": getRefValue(ref_start)}})], (_e0))}>
  <VStack>
  <Input id={`name`} placeholder={`Name`} ref={ref_name} type={`text`}/>
  <Input id={`email`} placeholder={`Email`} ref={ref_email} type={`text`}/>
  <Input id={`task`} placeholder={`Task`} ref={ref_task} type={`text`}/>
  <HStack>
  <Input id={`date`} placeholder={`Date`} ref={ref_date} type={`date`}/>
  <Input id={`start`} placeholder={`Time Start`} ref={ref_start} type={`time`}/>
  <Input id={`end`} placeholder={`Time End`} ref={ref_end} type={`time`}/>
</HStack>
  <HStack>
  <Checkbox id={`get`} ref={ref_get}>
  {`Clicking This Won't Add More Data`}
</Checkbox>
</HStack>
  <Button type={`submit`}>
  {`Submit`}
  ...
  <Divider/>
  <DataTableGrid columns={state.form_state.output.columns} data={state.form_state.output.data} pagination={true} search={true} sort={true}/>
</VStack>
</Box>
</Box>
  <Spacer/>
